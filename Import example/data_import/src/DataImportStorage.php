...
class DataImportStorage {
   ...
   * @see db_insert()
   */
  public static function insert($uid, $sku, $type, $status, $message, $vars = []) {
    $return_value = NULL;
...
//          '%query' => $e->query_string,
   ...
   * @see db_update()
   */
  public static function update($uid, $type, $sku, $status, $message, $vars = []) {
    try {
      // db_update()...->execute() returns the number of rows updated.
      $return_value = Database::getConnection();
      $return_value->update('data_import_table')
        ->fields(    array(
          'uid' => $uid,
          'type' => HTML::escape($type),
          'sku' => $sku,
   ...
   * @see db_delete()
   */
  public static function delete($uid, $type, $sku, $status, $message, $vars = []) {
//    $return_value = Database::getConnection();
//    $return_value->delete('data_import_table')
//      ->condition('sku', $sku)
//      ->execute();
  }
   ...
   */
  public static function load($uid, $type, $sku, $status, $message, $vars = []) {
    // Read all fields from the dbtng_example table.
    $return_value = Database::getConnection();
    $return_value->select('data_import_table','dit');
    $return_value->fields('d_import_t');
...
//    }
    ...
    return $return_value->execute()->fetchAll();
  }
...
//    $select = db_select('data_import_table', 'e');
//    // Join the users table, so we can get the entry creator's username.
//    $select->join('users_field_data', 'u', 'e.uid = u.uid');
//    // Select these specific fields for the output.
//    $select->addField('e', 'dii');
//    $select->addField('u', 'name', 'username');
//    $select->addField('e', 'message');
//    $select->addField('e', 'status');
//    $select->addField('e', 'timestamp');
//    // Filter only persons named "John".
//    $select->condition('e.name', 'import');
//    // Filter only persons older than 18 years.
// //   $select->condition('e.timestamp', 18, '=');
